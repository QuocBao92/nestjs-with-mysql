...
describe('SsoService', () => {
    ...
    beforeAll(async () => {
        ...
        decode = sinon.stub(JWT, 'decode').callsFake((jwt: string, options?: any) => {
            return { payload: { npi: 123, patient_ids: [{ id_type: 'EHRID', id: '123' }, { id_type: 'MR', id: '123' }] } };
        });
    ...
    describe('login', () => {
        it('login - Case No 16- Test login with error getValueByKey', async (done) => {
            const req: any = {
                headers: {
